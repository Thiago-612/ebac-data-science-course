...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Quantile bin feature / Faixa por quartis
...
def build_sample_weights(y: pd.Series) -> np.ndarray:
    """
    EN: Create sample weights emphasizing rare classes.
    PT: Cria pesos por amostra dando mais importância às classes raras.
    """
    class_weight_map = {
        3: 6.0,
        4: 3.0,
    ...
    return y.map(class_weight_map).astype(float).values
...
def fit_model(model, x_train, y_train, sample_weight=None):
    """
    EN: Fit model, using sample_weight when supported.
    PT: Treina o modelo, usando sample_weight quando suportado.
    """
...
def evaluate_model_train_test(
    ...
    """
    fitted_model = fit_model(model, x_train, y_train, sample_weight=sample_weight)
...
def save_comparison_csv(df: pd.DataFrame, filepath: Path) -> None:
    """
    EN: Save DataFrame as CSV.
    PT: Salva DataFrame em CSV.
    """
...
def main() -> None:
    """
    EN: Main advanced pipeline with boosting models.
    PT: Pipeline avançado principal com modelos boosting.
    """
    ...
    x_train_sel, x_test_sel, selection_importance_df = select_features_by_importance(
        x_train=x_train_fe,
        y_train=y_train,
        x_test=x_test_fe,
        top_n=12,
    ...
    save_comparison_csv(
        selection_importance_df,
        outputs_dir / "feature_selection_importance.csv"
    )

    # 5) Sample weights for rare classes
    sample_weights_train = build_sample_weights(y_train)
    ...
    for model_name, model in models.items():
        ...
        use_sample_weight = model_name in {"XGBoost", "LightGBM", "CatBoost"}
        ...
        result = evaluate_model_train_test(
            ...
            y_test=y_test,
            model_name=model_name,
            sample_weight=sample_weights_train if use_sample_weight else None,
        )
    ...
    save_comparison_csv(
        comparison_df,
        outputs_dir / "base_model_comparison_v3.csv"
    )

    # 8) Best base model
    best_base_model_name = comparison_df.iloc[0]["model_name"]
    best_base_model = fitted_models[best_base_model_name]
    best_base_result = detailed_results[best_base_model_name]

    print("\n==== BEST BASE MODEL ====\n")
    print(best_base_model_name)

    # 9) Tune RandomForest for comparison
    ...
    tuned_rf.fit(x_train_sel, y_train)

    y_pred_train_rf = tuned_rf.predict(x_train_sel)
    y_pred_test_rf = tuned_rf.predict(x_test_sel)

    tuned_train_metrics = calculate_metrics(y_train, y_pred_train_rf)
    tuned_test_metrics = calculate_metrics(y_test, y_pred_test_rf)
    ...
    save_metrics_json(
        ...
        outputs_dir / "tuned_random_forest_metrics_v3.json"
    )
    ...
    final_comparison = pd.DataFrame([
        {
            ...
            "cv_f1_macro": comparison_df.iloc[0]["cv_f1_macro"],
            "test_f1_macro": comparison_df.iloc[0]["test_f1_macro"],
        },
        {
    ...
    save_comparison_csv(
        final_comparison,
        outputs_dir / "final_comparison_v3.csv"
    )

    # 11) Save outputs from best base model
    best_y_pred_train = best_base_result["y_pred_train"]
    best_y_pred_test = best_base_result["y_pred_test"]

    save_classification_report(
        y_true=y_train,
        y_pred=best_y_pred_train,
        filepath=outputs_dir / "classification_report_train_best_model_v3.txt"
    )

    save_classification_report(
        y_true=y_test,
        y_pred=best_y_pred_test,
        filepath=outputs_dir / "classification_report_test_best_model_v3.txt"
    )

    save_confusion_matrix_figure(
        y_true=y_train,
        y_pred=best_y_pred_train,
        filepath=outputs_dir / "confusion_matrix_train_best_model_v3.png",
        title=f"Confusion Matrix - Train - {best_base_model_name}"
    )

    save_confusion_matrix_figure(
        y_true=y_test,
        y_pred=best_y_pred_test,
        filepath=outputs_dir / "confusion_matrix_test_best_model_v3.png",
        title=f"Confusion Matrix - Test - {best_base_model_name}"
    )

    save_feature_importance(
        model=best_base_model,
        feature_names=x_train_sel.columns.tolist(),
        filepath_csv=outputs_dir / "feature_importance_best_model_v3.csv",
        filepath_png=outputs_dir / "feature_importance_best_model_v3.png",
        top_n=15
    )
    ...
    final_summary = {
        ...
        "best_base_model_test_f1_macro": float(comparison_df.iloc[0]["test_f1_macro"]),
        "best_base_model_cv_f1_macro": float(comparison_df.iloc[0]["cv_f1_macro"]),
        "tuned_random_forest_test_f1_macro": float(tuned_test_metrics["f1_macro"]),
        "tuned_random_forest_cv_f1_macro": float(rf_search.best_score_),
        "selected_features": x_train_sel.columns.tolist(),
    }

    save_metrics_json(
        final_summary,
        outputs_dir / "final_summary_v3.json"
    )
