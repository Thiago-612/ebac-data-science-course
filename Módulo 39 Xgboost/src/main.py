...
def train_xgboost_baseline(X_train, y_train):
    """
    Treina um modelo XGBoost baseline.
    """
...
def get_xgboost_feature_importance(model, feature_names):
    """
    Retorna a importância das features de um modelo XGBoost treinado.
    """
...
def full_xgboost_feature_workflow(
    ...
    print("\n==== BLOCO 2: XGBOOST COM TOP FEATURES ====\n")

    X_top = X[top_features]

    top_features_results = run_xgboost_experiment(
        X=X_top,
        y=y,
    ...
    if (
        ...
        best_model = top_features_results["baseline_model"]
